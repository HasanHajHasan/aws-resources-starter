...
export async function startECSServiceTasks(
  region: string,
  resourcesARN: string[]
) {
  ...
  try {
    if (!resourcesARN.length) {
      info("No resources found with the specified tags.");
      return;
    }
    for (const resourceARN of resourcesARN) {      
      if (isECSService(resourceARN)) {
        ...
        if (runningCount == 0) {
          ...
          info(
            `Update command issued to set desired tasks to 1 for service ${serviceName} in cluster ${clusterName}.`
          );
        }
      }
    }
    ...
    setFailed(
      `Error updating ECS service ${serviceName} in cluster ${clusterName}: ${error.message}`
    );
  }
}
